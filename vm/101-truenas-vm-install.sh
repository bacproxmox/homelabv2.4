...
# Canonical Homelab v2.4 hardware IDs. Do not ask unless hardware really changed.
...
🧊 Homelab v2.4 - TrueNAS VM 101 oluşturma
Storage              : $STORAGE
...
3) Kurulum bitince Install Menu seçenek 4'e geç.
   Seçenek 4 içindeki post-install helper otomatik olarak şunları yapacak:
     qm stop 101
     qm set 101 --ide2 none
     qm set 101 --boot order=scsi0
     qm start 101
4) Router DHCP reservation önerisi:
   ${TRUENAS_FIXED_MAC} -> 192.168.50.101
   Bu reservation varsa TrueNAS ilk boot sonrası direkt 192.168.50.101 alır.
5) TrueNAS WebUI: http://192.168.50.101
6) TrueNAS WebUI > System Settings > Services > SSH:
   - Allow Password Authentication: ON
   - Password Login Groups: builtin_administrators veya truenas_admin admin grubu
   - Save, SSH Start, istersen Start Automatically
7) Sonra Proxmox install menu seçenek 4'ü çalıştır.
   API key elle istenmez; post-install helper tank/private import edip
   /root/homelab-secrets/truenas-api.env dosyasını oluşturur.
NEXT
