...
# Homelab v2.4 - TrueNAS post-install helper
...
say "============================================================"
say " Homelab v2.4 - TrueNAS post-install importfix v8"
say "============================================================"
say "Log: $LOG"
say
...
if ! qm status "$VMID" >/dev/null 2>&1; then
  fail "VM${VMID} bulunamadı."
fi

if qm status "$VMID" | grep -q "status: running"; then
  say "VM${VMID} çalışıyor, stop ediliyor..."
  qm stop "$VMID" || true
  sleep 3
fi
...
ssh-keygen -R "$TRUENAS_IP" >/dev/null 2>&1 || true
...
"${SSH_BASE[@]}" "${TRUENAS_USER}@${TRUENAS_IP}" \
  "SUDO_PASS_B64='${PASS_B64}' FINAL_IP='${FINAL_IP}' API_USER='${TRUENAS_USER}' TANK_GUID_DEFAULT='${TANK_GUID_DEFAULT}' PRIVATE_GUID_DEFAULT='${PRIVATE_GUID_DEFAULT}' bash -s" <<'REMOTE'
set -euo pipefail
...
import_pool_direct() {
  ...
  set +e
  srun midclt call pool.import_pool "{\"guid\":\"${guid}\"}"
  rc=$?
  set -e
...
if ! is_pool_active "tank" || ! is_pool_active "private"; then
  fail "tank/private import tamamlanmadı. API/network aşamasına geçilmiyor."
fi
